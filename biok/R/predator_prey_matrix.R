#' predator_prey_matrix Function
#'
#' This function returns a matrix corresponding to a network with predator/prey interactions
...
#' @export
predator_prey_matrix = function(S,mean,sd,C){
  may = diag(-1,S,S)
  for (i in 1:(S-1)){
    for (j in (i+1):S){
      if (runif(1)<=C){
